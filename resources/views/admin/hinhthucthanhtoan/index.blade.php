    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-4">
        <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active">Hình thức thanh toán</li>
        </ol>
      </div>
    </div>
  </div><!-- /.container-fluid -->
    ...
    <div class="col-12">
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">
            <a href="#" class="btn btn-info" data-toggle="modal" data-target="#exampleModal">Thêm hình thức thanh toán</a>
          </h3>
        ...
        <div class="card-body table-responsive p-0">
          <table class="table table-hover text-nowrap" id="category_table">
            ...
            <tbody>
              <?php $stt = 1; ?>
              @foreach ($hinhthucthanhtoan as $item => $value)
              <tr>
                <td>{{$stt++}}</td>
                <td>{{$value->httt_id}}</td>
                <td>{{$value->httt_ten}}</td>
                <td>{{$value->created_at}}</td>
                <td>{{$value->updated_at}}</td>
                <td>
                  {{-- <a href="{{ route('xoaxuatxu', ['id'=>$value->httt_id]) }}" class="btn btn-default">Chi tiết</a> --}}
                  <a href="{{ route('suahinhthucthanhtoan', ['id'=>$value->httt_id]) }}" class="btn btn-primary">Sửa</a>
                  <a href="{{ route('xoahinhthucthanhtoan', ['id'=>$value->httt_id]) }}" class="btn btn-danger">Xóa</a>
                </td>
              </tr>
              @endforeach
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form method="POST" action="{{ route('themhinhthucthanhtoan') }}">
              @csrf
              <div class="form-group">
                <label for="exampleInputEmail1">Tên hình thức thanh toán</label>
                <input name="tenHTTT" type="text" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Nhập tên hình thức thanh toán . . . ">
              </div>
